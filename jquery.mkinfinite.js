...
(function ($) {
  jQuery.fn.mkinfinite = function (options) {
    ...
    var make = function () {
      ...
      animationStep =
        (options.maxZoom - 1) /
        (options.animationTime / options.animationInterval);
      if (!options.zoomIn) {
    ...
    var calculateZoom = function (setImageNumber) {
      var nBGw =
        (options.isFixedBG ? $(window).width() : $object.width()) * currentZoom;
      ...
      if (setImageNumber && setImageNumber > 0) {
        $object.css({
          ...
          "background-image":
            "url(" + options.imagesList[setImageNumber - 1] + ")",
        });
        var imgLoader = new Image();
        imgLoader.src =
          options.imagesList[setImageNumber % options.imagesList.length];
      } else {
    ...
    var animateBG = function () {
      if (
        (options.zoomIn && currentZoom >= 1) ||
        (!options.zoomIn && currentZoom <= options.maxZoom)
      ) {
        setTimeout(function () {
          calculateZoom();
